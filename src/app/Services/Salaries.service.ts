...
export class SalariesService {
  ...
  create(data: any): Observable<Salaries> {
    return this.http.post<Salaries>("http://localhost:7080/api/salaries/save", data).pipe(
      map(response => response)
    );
  }
  ...
  getSalaries(id: number): Observable<Salaries> {
    return this.http.get<Salaries>("http://localhost:7080/api/salaries/save").pipe(
      map(response => response)
    )
  }
  ...
  uploadFile(formData: FormData):Observable<any> {
    return this.http.post('http://localhost:7080/api/v1/test/justificatif/fill-pdf', formData);
  }



  getPdfs(): Observable<any[]> {
    return this.http.get<any[]>("http://localhost:7080/api/pdf").pipe(
      map(response => response)
    );
  }
